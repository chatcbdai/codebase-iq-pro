...
claude mcp add --scope user git \
  /opt/homebrew/bin/uvx mcp-server-git
...
# Time
claude mcp add --scope user time \
  /opt/homebrew/bin/uvx mcp-server-time

# Fetch
claude mcp add --scope user fetch \
  /opt/homebrew/bin/uvx mcp-server-fetch
...
claude mcp add --scope user youtube-transcript \
  /opt/homebrew/bin/uvx mcp-youtube-transcript

# CodebaseIQ Pro
claude mcp add --scope user codebase-iq-pro \
  python3 /Users/chrisryviss/codebase_iq_pro/codebaseiq_pro.py \
  --env PYTHONPATH=/Users/chrisryviss/codebase_iq_pro/src \
